define(["jquery","jqext"],function($){
	...
	/**
	 * @class dw
	 * pin 类是所有dw JavaScript交互应用的命名空间根目录。
	 */
	$.extend(pin,{

		chkLogin:function(el){
			var isLogin = pin.getCfg("isLogin");
			if(isLogin){
				return true;
			} else {
				var obj = {};
				if(el && jQuery(el).attr("t")){
					obj.t = jQuery(el).attr("t");
				}
				pin.fire("needLogin",obj);
				return false;
			}
		},
		...
		getCfg:function(name){
			if(name == "userId"){
				if(pin.cfg && pin.cfg[name]) {
					return pin.cfg && pin.cfg[name];
				}else {
					return $.cookie('statLoginUserName');
				}
			}

			return (pin.cfg && pin.cfg[name])||(window.PageData&&window.PageData[name]);
		},
		...
		* @param {Function} callback
		*/
		unon:function(eName,callback){
			var eArr = this.event[eName];
			if(eArr){
				for(var i=0 ;i < eArr.length ;i++){
					if(eArr[i].d == callback){
						pin.util.arrayRemove(eArr,eArr[i]);
						i--;
					}
				}
			}
	...
	var Util = pin.util = {
		windowLogin:null,
		loginCallBack_crossDomain_ok : function(url) {
			location.href=url;
		},
		loginCallBack_crossDomain_error : function(msg) {
			if(pin.ui&&pin.ui.MsgBox){
				pin.ui.MsgBox.alert("",msg);
			}else{
				alert(msg);
			}
		...
		login:function(url){
				if (Util.windowLogin){
					 Util.windowLogin.close();
				}

				url += "?callback=" + encodeURIComponent( pin.getCfg("loginCallback") || location.href );
				Util.windowLogin = Util.openwindow(url,"_blank",750,550);
		},
		openwindow : function (url,name,iWidth,iHeight)
		{
				...
				var iHeight;                         //弹出窗口的高度;
				//获得窗口的垂直位置
				var iTop = (window.screen.availHeight-50-iHeight)/3;       
				//获得窗口的水平位置
				var iLeft = (window.screen.availWidth-10-iWidth)/2;
				return window.open(url,name)
				//return window.open(url,name,'height='+iHeight+',,innerHeight='+iHeight+',width='+iWidth+',innerWidth='+iWidth+',top='+iTop+',left='+iLeft+',status=yes,toolbar=no,menubar=no,location=yes,resizable=no,scrollbars=0,titlebar=no,depended=yes,Resizable=no');
		},

		loginCallBack:function(rv){
				if(rv){
					if(rv.msg == "ok"){
						var url = rv.login_back
						if(url.split('#').length == 2){
							url = url.substr(0,url.lastIndexOf('#'))
						}
						if(pin.util.dequery(url)._!=undefined){
							var len = url.length-2;
							if(url[url.length-1] == "&") len = url.length-3;
							if(url[url.length-1] == "?") len = url.length-4;
							url = url.substr(0,len);
						} else {
							url = url + (url.indexOf("?") != -1 ? "&" :"?") + "_"
						}
						location.href=url;
					} else {
						if(pin.ui.MsgBox)
							pin.ui.MsgBox.alert("",rv.msg)
						else
							alert(rv.msg);
					}
				}
		...
		share:function(config){
	        ...
	        jQuery.getScript('http://bdimg.share.baidu.com/static/api/js/share.js?v=89860593.js?cdnversion='+~(-new Date()/36e5),1);
	    },
				...
				ajax.onreadystatechange = function(){
					if (ajax.readyState === 4){
						...
						if(ok && cfg.success){
							...
							if(cfg.failure){
								cfg.failure.call(this,ajax);
								if(__debug){
									console.error("请求超时,",ajax);
								}
							}
						}
					}
		...
		create : function(){
			...
			var obj, basePro, ags = [].slice.call(arguments);
	...
	//request
	(function(pin,$){
		var Req = pin.request = pin.request||{};
		...
		Req.RequestData.prototype = {
			...
			getMsg:function(defaultValue){
				...
				return code && pin.request.ERRORMSG[code] || defaultValue || "请求不给力哦！";
			},
			/**
			 * 异步请求发送的数据
			 * @return {String}
